...
//const contenaire_offre_v2 = document.getElementById("contenaire_offre_v2");
const presentation_offre_modifier_v2 = document.getElementById("presentation_offre_modifier_v2");
const competence_offre_modifier_v2 = document.getElementById("competence_offre_modifier_v2");
const avantage_offre_modifier_v2 = document.getElementById("avantage_offre_modifier_v2");
const bouton_enregistrer_v2 = document.getElementById("bouton_enregistrer_v2");
const bouton_annuler_v2 = document.getElementById("bouton_annuler_v2");
const comfirmation_formulaire_offre_v2 = document.getElementById("comfirmation_formulaire_offre_v2");
const contenaire_offre_modifier_v2 = document.getElementById("contenaire_offre_modifier_v2");
const formulaire_offre_v2 = document.getElementById("formulaire_offre_v2");
...
function activer_modification_offre ()
{
    formulaire_offre_v2.style.display = "flex";
    contenaire_offre.style.display = "none";
    //contenaire_offre_v2.style.display = "flex";
    presentation_offre.style.display = "none";
    presentation_offre_modifier_v2.style.display = "flex";
    competence_offre.style.display = "none";
    competence_offre_modifier_v2.style.display = "flex";
    avantage_offre.style.display = "none";
    avantage_offre_modifier_v2.style.display = "flex";
    comfirmation_formulaire_offre_v2.style.display = "flex";
    //resumer_offre.style.display = "none";
    contenaire_offre_modifier_v2.style.display = "flex";

    //pour debugger
    //console.log("activer_creation_offre");
}

function desactiver_modification_offre ()
{
    formulaire_offre_v2.style.display = "none";
    contenaire_offre.style.display = "block";
    //contenaire_offre_v2.style.display = "none";
    presentation_offre.style.display = "flex";
    presentation_offre_modifier_v2.style.display = "none";
    competence_offre.style.display = "flex";
    competence_offre_modifier_v2.style.display = "none";
    avantage_offre.style.display = "flex";
    avantage_offre_modifier_v2.style.display = "none";
    comfirmation_formulaire_offre_v2.style.display = "none";
    //resumer_offre.style.display = "block";
    contenaire_offre_modifier_v2.style.display = "none";
...
bouton_modifier.addEventListener("click", activer_modification_offre);
...
bouton_annuler_v2.addEventListener("click", desactiver_modification_offre);
...
const message_erreur_offre_v2 = document.getElementById("message_erreur_offre_v2");
...
const champ_ville_v2 = document.getElementById("champ_ville_v2");
...
validation_champ_ville_candidature, validation_champ_numero_telephone_candidature, 
validation_champ_ville_v2 = false;
...
function verif_ville_v2(event)
{
    // que des lettres accentué (minuscule et majuscule)
    const ville_valider_v2 = /^[\p{L} -]+$/u;
    let valeur_champ_ville_v2 = champ_ville_v2.value;

    if (ville_valider_v2.test(valeur_champ_ville_v2))
    {
        validation_champ_ville_v2 = true;

        // pour debugger
        //console.log("verif_ville_v2 : if (ville_valid_v2.test(valeur_champ_ville_v2))");
        //console.log("ville valide_v2 = " + valeur_champ_ville_v2);
    }
    
    else
    {
        message_erreur_offre_v2.innerHTML = "Veuillez saisir un nom de ville valide !"
        validation_champ_ville_v2 = false;
        event.preventDefault();

        // pour debugger
        //console.log("verif_ville_v2 : else");
        //console.log("ville invalide = " + valeur_champ_ville_v2);
    }
}
...
bouton_enregistrer_v2.addEventListener("click", verif_ville_v2);
...
document.addEventListener("DOMContentLoaded", () => {
    const button = document.querySelector('#bouton_wish_list');
    const bouton_postuler = document.getElementById("bouton_postuler");
    const bouton_modifier = document.getElementById("bouton_modifier");
    const bouton_poubelle = document.getElementById("bouton_poubelle");
    
    // Si le bouton n'existe pas (pas d'offre sélectionnée), on sort
    if (!button) return;

    const roles = button.dataset.role.split(',');

    if (!roles.includes('etudiant') && !roles.includes('admin')) {
    console.log("Rôle non autorisé : ", roles);
    button.style.display = 'none';
    bouton_postuler.style.display = 'none';
    return;
    }

    if (!roles.includes('pilote') && !roles.includes('admin')) {
        console.log("Rôle non autorisé : ", roles);
        outon_modifier.style.display = 'none';
        bouton_poubelle.style.display = 'none';
        return;
        }

    // Écouteur d'événement si autorisé
    button.addEventListener('click', () => {
        const idOffre = button.dataset.idOffre;

        fetch('/app/Controllers/WishlistController.php?action=add', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ id_offre: parseInt(idOffre) })
        })
        .then(res => res.json())
        .then(data => {
            alert(data.success ? "✅ Ajoutée à la wishlist !" : "⚠️ " + data.message);
        })
        .catch(err => {
            console.error("❌ Erreur lors de l'ajout :", err);
            alert("❌ Problème lors de l'ajout à la wishlist.");
        });
    });
});
